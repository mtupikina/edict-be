...
## Cursor rules & commit gate

This repo follows workspace and project Cursor rules (see `.cursor/rules/` at workspace root and `edict-be/.cursor/rules/`):

- **Lint:** Run `npm run lint` from `edict-be/`. No errors or warnings are allowed.
- **Tests:** Run `npm run test` and `npm run test:e2e` (e2e when applicable). All must pass.
